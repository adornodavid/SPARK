...
import {
  extraerLoteProductFields,
  obtenerProductFieldsSupabase,
  conteoProductFieldsSupabase,
  conteoPipedriveProductFields,
  verificarPipedriveProductFields,
...
export default function ProductFieldPage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerProductFieldsSupabase(pagina, porPagina, busqueda)
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveProductFields()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteProductFields(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay product field para extraer desde Pipedrive.")
          break
        }
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Product Field</h1>
          <p className="text-muted-foreground mt-1">Extracción de product field desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Product Field en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Product Field en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Product Field en Supabase</CardTitle>
            <div className="flex items-center gap-2">
