...
import type { ErrorDetalle, TaskRow } from "@/app/actions/pipedrive"
import {
  extraerLoteTasks,
  obtenerTasksSupabase,
  conteoTasksSupabase,
  conteoPipedriveTasks,
...
export default function TasksPage() {
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveTasks()
    setConteoPipe(t)
    ...
    try {
      ...
      while (true) {
        ...
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay tasks para extraer desde Pipedrive.")
          break
        }
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Tasks en Pipedrive</p>
                ...
                <Table className="table-fixed w-full">
                  <TableHeader>
                    <TableRow className="border-border/50">
                      ...
                      <TableHead className="w-20">Asignado</TableHead>
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-12">No hay tasks registradas</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          ...
                          <TableCell className="text-sm">{item.proyecto_id || "—"}</TableCell>
                          <TableCell className="text-sm">{item.asignado_id || "—"}</TableCell>
