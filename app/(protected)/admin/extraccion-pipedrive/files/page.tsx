...
import type { ErrorDetalle, FileRow } from "@/app/actions/pipedrive"
import {
  extraerLoteFiles,
  obtenerFilesSupabase,
  conteoFilesSupabase,
  conteoPipedriveFiles,
  verificarPipedriveFiles,
...
export default function FilePage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerFilesSupabase(pagina, porPagina, busqueda)
    setItems(result.data as FileRow[])
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveFiles()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteFiles(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay files para extraer desde Pipedrive.")
          break
        }
    ...
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Extracción de archivos desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Files en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Files en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Files en Supabase</CardTitle>
            <div className="flex items-center gap-2">
            ...
            <>
              <div className="rounded-xl border border-border/50 overflow-x-auto">
                <Table className="table-fixed w-full">
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-12">No hay archivos registrados</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          ...
                          <TableCell className="text-sm">{item.file_size != null ? item.file_size.toLocaleString() : "—"}</TableCell>
