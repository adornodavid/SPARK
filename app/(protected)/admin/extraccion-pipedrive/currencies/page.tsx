...
import type { ErrorDetalle, CurrencyRow } from "@/app/actions/pipedrive"
import {
  extraerLoteCurrencies,
  obtenerCurrenciesSupabase,
  conteoCurrenciesSupabase,
  conteoPipedriveCurrencies,
  verificarPipedriveCurrencies,
...
export default function CurrencyPage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerCurrenciesSupabase(pagina, porPagina, busqueda)
    setItems(result.data as CurrencyRow[])
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveCurrencies()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteCurrencies(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay currencies para extraer desde Pipedrive.")
          break
        }
    ...
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Extracción de monedas desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Currencies en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Currencies en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Currencies en Supabase</CardTitle>
            <div className="flex items-center gap-2">
                ...
                <Table className="table-fixed w-full">
                  <TableHeader>
                    <TableRow className="border-border/50">
                      ...
                      <TableHead className="w-20">Símbolo</TableHead>
                      <TableHead className="w-24">Decimales</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-12">No hay monedas registradas</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          ...
                          <TableCell className="text-sm truncate">{item.nombre || "—"}</TableCell>
                          <TableCell className="text-sm">{item.simbolo || "—"}</TableCell>
                          <TableCell className="text-sm">{item.decimales ?? "—"}</TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
