...
import type { ErrorDetalle, LeadRow } from "@/app/actions/pipedrive"
import {
  extraerLoteLeads,
  obtenerLeadsSupabase,
  conteoLeadsSupabase,
  conteoPipedriveLeads,
  verificarPipedriveLeads,
...
export default function LeadPage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerLeadsSupabase(pagina, porPagina, busqueda)
    setItems(result.data as LeadRow[])
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveLeads()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteLeads(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay leads para extraer desde Pipedrive.")
          break
        }
    ...
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Extracción de leads desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Leads en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Leads en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Leads en Supabase</CardTitle>
            <div className="flex items-center gap-2">
                ...
                <Table className="table-fixed w-full">
                  <TableHeader>
                    <TableRow className="border-border/50">
                      ...
                      <TableHead className="w-24">Pd</TableHead>
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-12">No hay leads registrados</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          <TableCell className="text-sm text-muted-foreground">{item.id}</TableCell>
                          <TableCell className="text-xs text-muted-foreground truncate">{(item.pipedrive_id || "").slice(0, 8)}</TableCell>
                          <TableCell className="font-medium truncate">{item.titulo || "—"}</TableCell>
                          <TableCell className="text-sm truncate">{item.valor && typeof item.valor === "object" ? `${item.valor.amount || ""} ${item.valor.currency || ""}` : "—"}</TableCell>
                          <TableCell className="text-sm truncate">{item.origen_nombre || "—"}</TableCell>
                          <TableCell><span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${item.archivado ? "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-400 dark:border-amber-800" : "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950 dark:text-emerald-400 dark:border-emerald-800"}`}>{item.archivado ? "Archivado" : "Activo"}</span></TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
