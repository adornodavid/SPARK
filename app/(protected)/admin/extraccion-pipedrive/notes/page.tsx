...
import {
  extraerLoteNotes,
  obtenerNotesSupabase,
  conteoNotesSupabase,
  verificarPipedriveNotes,
} from "@/app/actions/pipedrive"
import type { NoteRow } from "@/app/actions/pipedrive"
...
export default function NotesPage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerNotesSupabase(pagina, porPagina, busqueda)
  ...
  // Limpiar HTML para mostrar texto plano en la tabla
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteNotes(start)
        ...
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay notas para extraer desde Pipedrive.")
          break
        }
  ...
  // Paginación
  const totalPaginas = Math.ceil(totalNotes / porPagina)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Extracción de notas desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Notes en Supabase</p>
              <p className="text-2xl font-bold">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Notes en Supabase</CardTitle>
            <div className="flex items-center gap-2">
              <Input
                placeholder="Buscar por contenido, persona u organización..."
                value={busqueda}
            ...
            <>
                ...
                <Table>
                  ...
                  <TableBody>
                    {notes.length === 0 ? (
                      ...
                      notes.map((note) => (
                        <TableRow key={note.id} className="border-border/50">
                          <TableCell className="text-sm text-muted-foreground">{note.pipedrive_id}</TableCell>
                          <TableCell className="text-sm max-w-[250px] truncate">{limpiarHtml(note.contenido)}</TableCell>
                          <TableCell className="text-sm">{note.persona_nombre || "—"}</TableCell>
                          <TableCell className="text-sm">{note.organizacion_nombre || "—"}</TableCell>
                          <TableCell className="text-sm">{note.trato_titulo || (note.trato_id ? `#${note.trato_id}` : "—")}</TableCell>
                          <TableCell className="text-sm">{note.usuario_nombre || "—"}</TableCell>
                          <TableCell className="text-sm">{note.pipedrive_add_time ? new Date(note.pipedrive_add_time).toLocaleDateString() : "—"}</TableCell>
                          <TableCell>
                            <span
              ...
              {totalPaginas > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <p className="text-sm text-muted-foreground">
                    Mostrando {((pagina - 1) * porPagina) + 1}-{Math.min(pagina * porPagina, totalNotes)} de {totalNotes}
                  </p>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
