...
import type { ErrorDetalle, NoteRow } from "@/app/actions/pipedrive"
import {
  extraerLoteNotes,
  obtenerNotesSupabase,
  conteoNotesSupabase,
  conteoPipedriveNotes,
...
export default function NotesPage() {
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveNotes()
    setConteoPipe(t)
    ...
    try {
      ...
      while (true) {
        ...
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay notes para extraer desde Pipedrive.")
          break
        }
  ...
  function contenidoCorto(c: string | null): string {
    if (!c) return "—"
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Notes en Pipedrive</p>
            ...
            <>
              <div className="rounded-xl border border-border/50 overflow-x-auto">
                <Table className="table-fixed w-full">
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-12">No hay notes registradas</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          ...
                          <TableCell className="text-sm truncate">{contenidoCorto(item.contenido)}</TableCell>
                          <TableCell className="text-sm truncate">{item.persona_nombre || "—"}</TableCell>
                          <TableCell className="text-sm truncate">{item.organizacion_nombre || "—"}</TableCell>
                          <TableCell className="text-sm truncate">{item.usuario_nombre || "—"}</TableCell>
                          <TableCell>
