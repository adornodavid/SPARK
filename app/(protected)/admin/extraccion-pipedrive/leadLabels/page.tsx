...
import type { ErrorDetalle, LeadLabelRow } from "@/app/actions/pipedrive"
import {
  extraerLoteLeadLabels,
  obtenerLeadLabelsSupabase,
  conteoLeadLabelsSupabase,
  conteoPipedriveLeadLabels,
  verificarPipedriveLeadLabels,
} from "@/app/actions/pipedrive"

export default function LeadLabelPage() {
  const router = useRouter()
  const cancelRef = useRef(false)

  const [items, setItems] = useState<LeadLabelRow[]>([])
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerLeadLabelsSupabase(pagina, porPagina, busqueda)
    setItems(result.data as LeadLabelRow[])
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveLeadLabels()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteLeadLabels(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay lead labels para extraer desde Pipedrive.")
          break
        }
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Lead Labels</h1>
          <p className="text-muted-foreground mt-1">Extracción de etiquetas de leads desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Lead Labels en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Lead Labels en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Lead Labels en Supabase</CardTitle>
            <div className="flex items-center gap-2">
            ...
            <>
              <div className="rounded-xl border border-border/50 overflow-x-auto">
                <Table className="table-fixed w-full">
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-12">No hay etiquetas registradas</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
