...
import type { ErrorDetalle, ActivityRow } from "@/app/actions/pipedrive"
import {
  extraerLoteActivities,
  obtenerActivitiesSupabase,
  conteoActivitiesSupabase,
  conteoPipedriveActivities,
...
export default function ActivitiesPage() {
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveActivities()
    setConteoPipe(t)
    ...
    try {
      ...
      while (true) {
        ...
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay activities para extraer desde Pipedrive.")
          break
        }
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Activities en Pipedrive</p>
                ...
                <Table className="table-fixed w-full">
                  <TableHeader>
                    <TableRow className="border-border/50">
                      ...
                      <TableHead className="w-20">Org.</TableHead>
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-12">No hay activities registradas</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          ...
                          <TableCell className="font-medium truncate">{item.asunto || "—"}</TableCell>
                          <TableCell className="text-sm truncate">{item.tipo || "—"}</TableCell>
                          <TableCell className="text-sm">{item.persona_id || "—"}</TableCell>
                          <TableCell className="text-sm">{item.organizacion_id || "—"}</TableCell>
                          <TableCell className="text-sm">{item.fecha_vencimiento || "—"}</TableCell>
                          <TableCell>
                            <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${
                              item.completado
                                ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950 dark:text-emerald-400 dark:border-emerald-800"
                                : "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-400 dark:border-amber-800"
                            }`}>{item.completado ? "Completado" : "Pendiente"}</span>
                          </TableCell>
                        </TableRow>
                      ))
                    )}
