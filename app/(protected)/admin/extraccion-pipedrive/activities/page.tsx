...
import {
  extraerLoteActivities,
  obtenerActivitiesSupabase,
  conteoActivitiesSupabase,
  verificarPipedriveActivities,
} from "@/app/actions/pipedrive"
import type { ActivityRow } from "@/app/actions/pipedrive"
...
export default function ActivitiesPage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerActivitiesSupabase(pagina, porPagina, busqueda)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteActivities(start)
        ...
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay actividades para extraer desde Pipedrive.")
          break
        }
  ...
  // Paginación
  const totalPaginas = Math.ceil(totalActivities / porPagina)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Extracción de actividades desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Activities en Supabase</p>
              <p className="text-2xl font-bold">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Activities en Supabase</CardTitle>
            ...
            <>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-border/50">
                      ...
                      <TableHead className="text-center">Persona ID</TableHead>
                      <TableHead className="text-center">Org ID</TableHead>
                      <TableHead className="text-center">Trato ID</TableHead>
                  ...
                  <TableBody>
                    {activities.length === 0 ? (
                      ...
                      activities.map((act) => (
                        <TableRow key={act.id} className="border-border/50">
                          <TableCell className="text-sm text-muted-foreground">{act.pipedrive_id}</TableCell>
                          <TableCell className="font-medium max-w-[200px] truncate">{act.asunto || "—"}</TableCell>
                          <TableCell className="text-sm">
                            <span className="inline-flex items-center rounded-full bg-blue-50 border border-blue-200 px-2.5 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-950 dark:text-blue-400 dark:border-blue-800">
                              {act.tipo || "—"}
                            </span>
                          </TableCell>
                          <TableCell className="text-sm">{act.fecha_vencimiento || "—"}</TableCell>
                          <TableCell className="text-sm">{act.hora_vencimiento || "—"}</TableCell>
                          <TableCell className="text-sm">{act.duracion || "—"}</TableCell>
                          <TableCell className="text-sm text-center">{act.persona_id || "—"}</TableCell>
                          <TableCell className="text-sm text-center">{act.organizacion_id || "—"}</TableCell>
                          <TableCell className="text-sm text-center">{act.trato_id || "—"}</TableCell>
                          <TableCell>
                            <span
              ...
              {totalPaginas > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <p className="text-sm text-muted-foreground">
                    Mostrando {((pagina - 1) * porPagina) + 1}-{Math.min(pagina * porPagina, totalActivities)} de {totalActivities}
                  </p>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
