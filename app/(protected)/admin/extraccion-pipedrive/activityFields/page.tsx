...
import {
  extraerLoteActivityFields,
  obtenerActivityFieldsSupabase,
  conteoActivityFieldsSupabase,
  conteoPipedriveActivityFields,
  verificarPipedriveActivityFields,
} from "@/app/actions/pipedrive"

export default function ActivityFieldPage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerActivityFieldsSupabase(pagina, porPagina, busqueda)
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveActivityFields()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteActivityFields(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay activity field para extraer desde Pipedrive.")
          break
        }
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Activity Field</h1>
          <p className="text-muted-foreground mt-1">Extracción de activity field desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Activity Field en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Activity Field en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Activity Field en Supabase</CardTitle>
            <div className="flex items-center gap-2">
