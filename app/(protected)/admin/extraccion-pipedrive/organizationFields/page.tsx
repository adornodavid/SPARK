...
import type { ErrorDetalle, FieldRow } from "@/app/actions/pipedrive"
import {
  extraerLoteOrganizationFields,
  obtenerOrganizationFieldsSupabase,
  conteoOrganizationFieldsSupabase,
  conteoPipedriveOrganizationFields,
  verificarPipedriveOrganizationFields,
} from "@/app/actions/pipedrive"

export default function OrganizationFieldPage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerOrganizationFieldsSupabase(pagina, porPagina, busqueda)
    setItems(result.data as FieldRow[])
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveOrganizationFields()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteOrganizationFields(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay organization field para extraer desde Pipedrive.")
          break
        }
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Organization Field</h1>
          <p className="text-muted-foreground mt-1">Extracción de organization field desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Organization Field en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Organization Field en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Organization Field en Supabase</CardTitle>
            <div className="flex items-center gap-2">
                ...
                <Table className="table-fixed w-full">
                  <TableHeader>
                    <TableRow className="border-border/50">
                      ...
                      <TableHead className="w-16">Pd</TableHead>
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-12">No hay campos registrados</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          ...
                          <TableCell className="text-sm truncate">{item.clave || "—"}</TableCell>
                          <TableCell className="text-sm">{item.tipo || "—"}</TableCell>
                          <TableCell><span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${item.obligatorio ? "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-400 dark:border-amber-800" : "bg-slate-50 text-slate-700 border-slate-200 dark:bg-slate-950 dark:text-slate-400 dark:border-slate-800"}`}>{item.obligatorio ? "Sí" : "No"}</span></TableCell>
                          <TableCell><span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${item.activo ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950 dark:text-emerald-400 dark:border-emerald-800" : "bg-red-50 text-red-700 border-red-200 dark:bg-red-950 dark:text-red-400 dark:border-red-800"}`}>{item.activo ? "Sí" : "No"}</span></TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
