...
import type { ErrorDetalle, PermissionSetRow } from "@/app/actions/pipedrive"
import {
  extraerLotePermissionSets,
  obtenerPermissionSetsSupabase,
  conteoPermissionSetsSupabase,
  conteoPipedrivePermissionSets,
  verificarPipedrivePermissionSets,
...
export default function PermissionSetPage() {
  ...
  const [items, setItems] = useState<PermissionSetRow[]>([])
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerPermissionSetsSupabase(pagina, porPagina, busqueda)
    setItems(result.data as PermissionSetRow[])
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedrivePermissionSets()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLotePermissionSets(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay permission sets para extraer desde Pipedrive.")
          break
        }
    ...
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Extracción de permission sets desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Permission Sets en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Permission Sets en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Permission Sets en Supabase</CardTitle>
            <div className="flex items-center gap-2">
                ...
                <Table className="table-fixed w-full">
                  <TableHeader>
                    <TableRow className="border-border/50">
                      ...
                      <TableHead className="w-28">Asignaciones</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-12">No hay permission sets</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          ...
                          <TableCell className="text-sm">{item.asignaciones_count ?? 0}</TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
