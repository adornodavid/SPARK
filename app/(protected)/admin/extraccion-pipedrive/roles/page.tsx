...
import type { ErrorDetalle, RoleRow } from "@/app/actions/pipedrive"
import {
  extraerLoteRoles,
  obtenerRolesSupabase,
  conteoRolesSupabase,
  conteoPipedriveRoles,
  verificarPipedriveRoles,
...
export default function RolePage() {
  ...
  const cargarTabla = useCallback(async () => {
    ...
    const result = await obtenerRolesSupabase(pagina, porPagina, busqueda)
    setItems(result.data as RoleRow[])
  ...
  async function validarConteoPipedrive() {
    ...
    const t = await conteoPipedriveRoles()
    setConteoPipe(t)
  ...
  async function iniciarExtraccion() {
    ...
    try {
      ...
      while (true) {
        ...
        const resultado = await extraerLoteRoles(start)
        if (!resultado.success) { toast.error(resultado.mensaje); break }
        if (resultado.total_lote === 0 && !resultado.hay_mas) {
          toast.success("No hay roles para extraer desde Pipedrive.")
          break
        }
    ...
    <div className="space-y-6 w-full max-w-full overflow-x-hidden">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Extracción de roles desde Pipedrive</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            <div className="rounded-full bg-blue-500/10 p-3 shrink-0"><Cloud className="h-5 w-5 text-blue-500" /></div>
            <div>
              <p className="text-sm text-muted-foreground">Roles en Pipedrive</p>
        ...
        <Card className="w-fit">
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Roles en Supabase</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle>Roles en Supabase</CardTitle>
            <div className="flex items-center gap-2">
            ...
            <>
              <div className="rounded-xl border border-border/50 overflow-x-auto">
                <Table className="table-fixed w-full">
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-12">No hay roles registrados</TableCell></TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.id} className="border-border/50">
                          ...
                          <TableCell className="text-sm">{item.assignment_count ?? 0}</TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
